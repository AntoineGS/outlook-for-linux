...
const LEGACY = { id: 'profile-0', partition: product.partition, name: 'My account' };
const PROFILE_A = {
  id: 'profile-a',
  partition: `${product.profilePartitionPrefix}a`,
  name: 'A',
};
...
function build(profiles) {
  ...
  const pvm = new ProfileViewManager(win, pm, { url: product.defaultUrl }, () => {});
  pvm.initialize();
  return { win, pm, pvm };
}
...
describe('ProfileViewManager sender attribution wiring', () => {
  ...
  it('removing a profile whose webContents already died does not throw and still clears its partition storage', () => {
    ...
    assert.strictEqual(
      partitionSessions[PROFILE_A.partition].clearCalls,
      1
    );
  });
