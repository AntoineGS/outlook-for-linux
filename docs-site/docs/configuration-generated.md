...
| `appTitle` | `string` | `"Microsoft Teams"` | A text to be suffixed with page title **Deprecated:** use app.title instead | `restart` |
...
| `enableIncomingCallToast` | `boolean` | `false` | Enable incoming call toast **Deprecated:** use incomingCalls.toast instead | `restart` |
| `followSystemTheme` | `boolean` | `false` | Follow the operating-system dark/light theme preference. Default is false; set true to drive Teams's theme from the OS preference. **Deprecated:** use appearance.followSystemTheme instead | `restart` |
...
| `incomingCallCommand` | `string` | `null` | Command to execute on an incoming call. (caution: "~" in path is not supported) **Deprecated:** use incomingCalls.command instead | `restart` |
| `incomingCallCommandArgs` | `array` | `[]` | Arguments for the incoming call command. **Deprecated:** use incomingCalls.commandArgs instead | `restart` |
| `isCustomBackgroundEnabled` | `boolean` | `false` | A flag indicates whether to enable custom background or not **Deprecated:** use customBackground.enabled instead | `restart` |
...
| `meetupJoinRegEx` | `string` | `"^https://teams\\.(?:microsoft\\.com\|live\\.com\|cloud\\.microsoft)/(v2/\\?meetingjoin=\|meet/\|l/(?:app\|call\|channel\|chat\|entity\|file\|meet(?:ing\|up-join)\|message\|task\|team)/)"` | Regex for Teams meetup-join and related links **Deprecated:** use urlHandling.meetupJoinRegEx instead | `restart` |
...
| `onNewWindowOpenMeetupJoinUrlInApp` | `boolean` | `true` | Open meetupJoinRegEx URLs in the app instead of the default browser **Deprecated:** use urlHandling.openMeetupJoinInApp instead | `restart` |
| `partition` | `string` | `"persist:teams-4-linux"` | BrowserWindow webpreferences partition **Deprecated:** use app.partition instead | `restart` |
...
| `url` | `string` | `"https://teams.cloud.microsoft"` | Microsoft Teams URL **Deprecated:** use app.url instead | `restart` |
...
| `customBackground` | `object` | `{"enabled":false,"serviceBaseUrl":"http://localhost","configFetchInterval":0}` | Custom background image configuration. Replaces the deprecated isCustomBackgroundEnabled, customBGServiceBaseUrl and customBGServiceConfigFetchInterval options. | `restart` |
| `urlHandling` | `object` | `{"defaultHandler":"","meetupJoinRegEx":"^https://teams\\.(?:microsoft\\.com\|live\\.com\|cloud\\.microsoft)/(v2/\\?meetingjoin=\|meet/\|l/(?:app\|call\|channel\|chat\|entity\|file\|meet(?:ing\|up-join)\|message\|task\|team)/)","openMeetupJoinInApp":true}` | How the app decides which URLs it opens itself and which it hands to the desktop. Replaces the deprecated defaultURLHandler, meetupJoinRegEx and onNewWindowOpenMeetupJoinUrlInApp options. | `restart` |
| `incomingCalls` | `object` | `{"toast":false,"command":null,"commandArgs":[]}` | Incoming call notification and external command configuration. Replaces the deprecated enableIncomingCallToast, incomingCallCommand and incomingCallCommandArgs options. | `restart` |
...
| `platform` | `object` | `{"chromeUserAgent":"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/undefined Safari/537.36","emulateWindowsChromium":false,"spellCheckerLanguages":[],"disableTimestampOnCopy":false}` | Platform emulation and OS integration configuration. Replaces the deprecated chromeUserAgent, emulateWinChromiumPlatform, spellCheckerLanguages and disableTimestampOnCopy options. | `restart` |
...
| `auth.webLogin.passwordCommand` | `string` | `""` | Command whose first stdout line is pre-filled into the web sign-in password field. Runs in a shell; use your own password manager (e.g. 'pass show teams'). Empty disables it. The app stores no secret. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `customBackground.enabled` | `boolean` | `false` | A flag indicates whether to enable custom background or not |
| `customBackground.serviceBaseUrl` | `string` | `"http://localhost"` | Base URL of the server which provides custom background images |
| `customBackground.configFetchInterval` | `number` | `0` | A numeric value in seconds as poll interval to download background service config download |
...
| `urlHandling.openMeetupJoinInApp` | `boolean` | `true` | Open meetupJoinRegEx URLs in the app instead of the default browser |
...
| `incomingCalls.command` | `string` | `null` | Command to execute on an incoming call. (caution: "~" in path is not supported) |
| `incomingCalls.commandArgs` | `array` | `[]` | Arguments for the incoming call command. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `platform.chromeUserAgent` | `string` | `"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/undefined Safari/537.36"` | Google Chrome User Agent |
...
| `platform.disableTimestampOnCopy` | `boolean` | `false` | Controls whether timestamps are included when copying messages in chats |

### app

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `app.title` | `string` | `"Microsoft Teams"` | A text to be suffixed with page title |
