...
| `appTitle` | `string` | `"Microsoft Outlook"` | A text to be suffixed with page title | `restart` |
...
| `partition` | `string` | `"persist:outlook-4-linux"` | BrowserWindow webpreferences partition | `restart` |
...
| `url` | `string` | `"https://outlook.office.com/"` | Microsoft Outlook URL | `restart` |
| `customUserDir` | `string` | `null` | Custom user data directory | `restart` |
...
| `shortcuts` | `object` | `{"global":[],"disableWhileFocused":[],"vim":{"enabled":false}}` | Keyboard shortcut configuration. global: global keyboard shortcuts that work system-wide, disabled by default (opt-in). disableWhileFocused: global shortcuts to disable while the app is in focus. vim.enabled: enable Vim-style Outlook navigation outside editable fields. Replaces the deprecated globalShortcuts and disableGlobalShortcuts options. | `restart` |
...
| `shortcuts.vim.enabled` | `boolean` | `false` | Enable Vim-style Outlook navigation outside editable fields. Disabled by default. |
